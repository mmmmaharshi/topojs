...
const monthly: { year: number; value: number }[] = raw.map((line) => {
  ...
  return { year: Number(m[1]), value: Number(m[2]) };
});
console.log(`Loaded ${monthly.length} real monthly sunspot readings ` +
...
if (LAG === 1) LAG = 3; // fallback if ACF never crosses in the scanned range
...
function meanMaxPersistence(points: number[][]): number {
  const s = new StreamingHomology({ windowSize: WINDOW_SIZE, dims: 2, maxDist: MAX_DIST, maxDim: 2 });
  let sum = 0;
  let total = 0;
...
console.log(`  window=${WINDOW_SIZE}  lag=${LAG}yr (data-driven)  maxDist=${MAX_DIST}  significance=${SIGNIFICANCE}`);
console.log('='.repeat(78));

const observed = meanMaxPersistence(embed(yearly));
console.log(`Observed statistic (real data, mean maxPersistenceH1 per window): ${observed.toFixed(5)}`);
...
console.log(`Null distribution (${N_PERMUTATIONS} shuffles): mean=${nullMean.toFixed(5)}  std=${nullStd.toFixed(5)}  ` +
...
if (pValue < 0.01) {
  ...
  console.log(`nonparametric permutation test (p=${pValue.toFixed(4)}, z=${zScore.toFixed(2)}, n=${N_PERMUTATIONS} shuffles).`);
