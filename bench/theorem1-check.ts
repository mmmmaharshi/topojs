...
function maxFiniteDeath(pairs: PersistencePair[]): number {
  let m = -Infinity;
  for (const p of pairs) if (p.death !== -1 && p.death > m) m = p.death;
  return m;
}
...
for (let trial = 0; trial < TRIALS; trial++) {
  ...
  for (const T of T_GRID) {
    for (const maxDim of [1, 2] as const) {
      if (dims === 3 && maxDim === 2 && trial % 3 !== 0) continue; // keep H2 sparse
      ...
      for (const dim of dimsToCheck) {
        ...
        if (!Number.isFinite(dI)) interiorFinite = false;
        if (!Number.isFinite(dF)) fullFinite = false;
        if (Number.isFinite(dI) && dI > dB_interior) dB_interior = dI;
        if (Number.isFinite(dF) && dF > dB_full) dB_full = dF;
      }
      if (!interiorFinite || !fullFinite) continue; // essential-count mismatch => ∞, same as boundary-sensitivity skip
      ...
      total++;
      if (isInterior) interiorCases++;
      else boundaryCases++;
      if (!passInterior) interiorViolations++;
      if (!passFull) theorem1bViolations++;
      if (passFull && isInterior) interiorByTheorem1a++;
...
console.log(`Trials: ${TRIALS} × T in [${T_GRID.join(", ")}], dims 2/3, n≈28-39, L≈8-13`);
...
console.log(`Interior d_B ≤ 2λ violations (Theorem 1b): ${interiorViolations}/${total}`);
console.log(`Full d_B ≤ 2λ+gap violations (Theorem 1a/b): ${theorem1bViolations}/${total}`);
console.log(`  of which exact-bound cases (Theorem 1a): ${interiorByTheorem1a} passed`);
console.log(`Max excess over 2λ observed: ${maxExcess.toFixed(6)} (must be < 2λ)`);
console.log("");
...
if (rows.length > 0) {
  ...
  for (const r of rows) {
    console.log(
      `  trial=${r.trial} dims=${r.dims} n=${r.n} L=${r.L} λ=${r.lambda.toFixed(4)} T*=${r.tStar.toFixed(4)} dB_int=${r.dB_interior.toFixed(4)} dB_full=${r.dB_full.toFixed(4)} ${r.interior ? "interior" : "boundary"} gap=${r.gap.toFixed(4)} bound=${r.bound.toFixed(4)} ${r.passInterior && r.passFull ? "PASS" : "FAIL"}`,
    );
  }
  console.log("");
}
...
if (interiorViolations === 0 && theorem1bViolations === 0) {
  ...
  console.log("  Cite as: Thm 1, paper/New_Theorem_Truncated_Stability_and_Incremental_Exactness.md");
} else {
  console.log("✗ Unexpected violations — inspect rows above (numerical tol 1e-9).");
  process.exitCode = 1;
}
