...
Auto mode selects `implicit-full` above crossover thresholds, otherwise `cohomology`/`standard`; `reduced` is opt-in for H₀+H₁ through `computePersistentHomology({engine:"reduced", maxDim:1})`.
