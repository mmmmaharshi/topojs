...
describe('SlidingWindow', () => {
  it('rejects invalid construction parameters', () => {
    expect(() => new SlidingWindow(0, 2)).toThrow();
    expect(() => new SlidingWindow(5, 0)).toThrow();
  });
  ...
  it('accumulates points below capacity without evicting', () => {
    ...
    expect(Array.from(w.toFlatArray())).toEqual([1, 1, 2, 2]);
  });
  ...
  it('evicts oldest point once capacity is reached', () => {
    ...
    expect(Array.from(w.toFlatArray())).toEqual([1, 2, 3]);
    w.push([4]); // evicts 1
    expect(w.size).toBe(3);
    expect(Array.from(w.toFlatArray())).toEqual([2, 3, 4]);
    w.push([5]); // evicts 2
    expect(Array.from(w.toFlatArray())).toEqual([3, 4, 5]);
  });

  it('rejects points of the wrong dimensionality', () => {
    const w = new SlidingWindow(3, 2);
    expect(() => w.push([1, 2, 3])).toThrow();
    expect(() => w.push([1])).toThrow();
  });
  ...
  it('handles capacity=1 (every push evicts the previous point)', () => {
    ...
    expect(Array.from(w.toFlatArray())).toEqual([2, 2]);
  });
});
...
describe('StreamingHomology (Phase A / naive)', () => {
  ...
  it('matches a fresh full recompute on the same window contents (differential test)', () => {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      expect(update.result.complex).toEqual(expected.complex);
    }
  });

  it('isFull flips true exactly when windowSize points have been pushed', () => {
    const s = new StreamingHomology({ windowSize: 4, dims: 2, maxDist: 5.0 });
    for (let i = 0; i < 3; i++) {
      s.push([i, i]);
  ...
  it('detects a synthetic topological event: a loop entering the window', () => {
    ...
    const windowSize = 12;
    const s = new StreamingHomology({ windowSize, dims: 2, maxDist: 0.6, maxDim: 2 });
    ...
    for (let i = 0; i < windowSize; i++) {
      ...
      const update = s.push(pt);
      if (update) lastSummaryNoise = summarizeForStreaming(update.result.pairs, SIGNIFICANCE);
    }
    ...
    for (let i = 0; i < windowSize; i++) {
      ...
      const update = s.push(pt);
      if (update) lastSummaryRing = summarizeForStreaming(update.result.pairs, SIGNIFICANCE);
    }
    expect(s.isFull).toBe(true);
...
describe('summarizeForStreaming', () => {
  it('ignores H0 and essential-vs-finite H1 are counted separately', () => {
    const s = summarizeForStreaming([
      { dim: 0, birth: 0, death: 5 }, // ignored
      { dim: 1, birth: 0, death: 0.5 }, // finite, persistence 0.5
      { dim: 1, birth: 0, death: -1 }, // essential
      { dim: 2, birth: 0, death: 1 }, // ignored (not H1)
    ]);
  ...
  it('significanceThreshold filters out low-persistence noise', () => {
    const pairs = [
      { dim: 1, birth: 0, death: 0.01 },
      { dim: 1, birth: 0, death: 0.5 },
    ];
