...
import { computePersistentHomologyCohomologyFromComplex, computePersistentHomologyCohomologyImplicit } from '../src/core/homology-cohom-implicit.ts';
...
function checkMatchesExact(points: Float64Array, dims: number, maxDist: number, maxDim: number): void {
...
function checkMatchesImplicitVsExplicit(points: Float64Array, dims: number, maxDist: number, maxDim: number): void {
  ...
  expect(implicit.complex).toEqual(explicit.complex);
...
describe('Sheehy sparse Rips (epsilon parameter on buildRipsComplex)', () => {
  it('epsilon=0 means no shearing (exact complex, no Sheehy metadata)', () => {
    ...
    for (let seed = 0; seed < 10; seed++) {
      const pts: [number, number][] = [];
      for (let i = 0; i < 25; i++) pts.push([rng(), rng()]);
      const flat = generatePoints(pts);
      const maxDist = 0.4;
      ...
      expect(zero.edges.length).toBe(full.edges.length);
      expect(zero.triangles.length).toBe(full.triangles.length);
    }
  });
  ...
  it('epsilon < 1 produces a sparser complex (fewer simplices)', () => {
    ...
    const pts: [number, number][] = [];
    for (let i = 0; i < 60; i++) pts.push([rng(), rng()]);
    const flat = generatePoints(pts);
    const maxDist = 0.35;
    const full = buildRipsComplex(flat, 2, maxDist, 2);
  ...
  it('with large enough epsilon to include all points, matches the exact engine exactly', () => {
    ...
    const pts: [number, number][] = [];
    for (let i = 0; i < 20; i++) pts.push([rng(), rng()]);
    const flat = generatePoints(pts);
    ...
    const sparseComplex = buildRipsComplex(flat, 2, maxDist, 2, 1.0);
