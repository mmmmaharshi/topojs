...
describe("computePersistentHomologyImplicit vs computePersistentHomologyCohomology", () => {
  ...
  for (const seed of seeds) {
    it(`seed=${seed}: H1 pairs match cohomology engine exactly`, () => {
      ...
      const maxDist = maxDistCandidates[
        Math.floor(rng() * maxDistCandidates.length)
      ]!;

      const cohomResult = computePersistentHomologyCohomology(pts, dims, maxDist, 2);
      const implicitResult = computePersistentHomologyImplicit(pts, dims, maxDist, 2);

      const cohomPairs = sortPairs(cohomResult.pairs.filter((p) => p.dim !== 2));
      const implicitPairs = sortPairs(implicitResult.pairs.filter((p) => p.dim !== 2));

      expect(implicitPairs).toStrictEqual(cohomPairs);
    });

    it(`seed=${seed}: H2 pairs match cohomology engine exactly`, () => {
      const rng = mulberry32(seed);
      const n = 5 + Math.floor(rng() * 26);
      const dims = 2 + Math.floor(rng() * 4);
      const pts = randomPoints(rng, n, dims);
      ...
      const cohomResult = computePersistentHomologyCohomology(pts, dims, maxDist, 3);
      const implicitResult = computePersistentHomologyImplicit(pts, dims, maxDist, 3);
