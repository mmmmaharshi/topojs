var N = class {
  ...
  constructor(t) {
    ...
    for (let e = 0; e < t; e++) this.parent[e] = -1;
  }
  find(t) {
    if (this.parent[t] < 0) return t;
    let s = t;
    for (; this.parent[s] >= 0;) s = this.parent[s];
    for (; t !== s;) {
      let r = this.parent[t];
      ((this.parent[t] = s), (t = r));
    }
  ...
  reset() {
    for (let t = 0; t < this.parent.length; t++) this.parent[t] = -1;
  }
};
function j(i, t) {
  let e = new N(i),
...
var H = class i {
  ...
  cellKeyForPoint(t, e) {
    ...
    for (let a = 0; a < this.dims; a++) {
      let o = this.cellCoord(t[s + a]) + i.BIAS;
      r = (r << 32n) | BigInt(o);
    }
    return r;
  }
  candidatesAfter(t, e) {
    ...
    for (let g = 0; g < this.dims; g++) r[g] = this.cellCoord(t[s + g]);
    let a = [],
      o = [-1, 0, 1],
    ...
    for (let g = 0; g < u; g++) {
      ...
      for (let d = 0; d < this.dims; d++) {
        let P = b % 3;
        b = Math.floor(b / 3);
        let A = r[d] + o[P] + i.BIAS;
        v = (v << 32n) | BigInt(A);
      }
      let l = this.buckets.get(v);
      if (l) for (let d of l) d > e && a.push(d);
    }
    return (a.sort((g, v) => g - v), a);
  }
};
function L(i, t, e, s) {
  let r = e * t,
...
function J(i, t, e, s = 2) {
  ...
  for (let n = 0; n < r; n++) {
    let h = g ? g.candidatesAfter(i, n) : null,
      p = (m) => {
        let y = L(i, t, n, m);
        y <= e &&
          (a.push({ u: n, v: m, val: y, origIdx: o[n].length }), o[n].push(m), o[m].push(n));
      };
    if (h) for (let m of h) p(m);
    else for (let m = n + 1; m < r; m++) p(m);
  }
  a.sort((n, h) => n.val - h.val || n.origIdx - h.origIdx);
  let v = a.map((n) => ({ u: n.u, v: n.v, val: n.val })),
  ...
  for (let n = 0; n < r; n++) o[n].sort((h, p) => h - p);
  let A = Math.ceil(r / 32),
    E = new Array(r);
  for (let n = 0; n < r; n++) {
    let h = new Uint32Array(A),
      p = o[n];
    for (let m = 0; m < p.length; m++) h[p[m] >>> 5] |= 1 << (p[m] & 31);
    E[n] = h;
  }
  let x = [];
  for (let n = 0; n < v.length; n++) {
    let { u: h, v: p, val: m } = v[n],
      y = E[h],
      w = E[p],
      k = (p + 1) >>> 5,
      S = (p + 1) & 31;
    for (let I = k; I < A; I++) {
      ...
      for (I === k && S > 0 && (D &= ~((1 << S) - 1)); D;) {
        ...
        let $ = P(h, F),
          ...
          z = v[C].val,
          _ = Math.max(m, U, z);
        x.push({ edges: [n, $, C], verts: [h, p, F], val: _ });
      }
    }
  }
      ...
      for (let M = I; M < A; M++) {
        ...
        for (M === I && D > 0 && (T &= ~((1 << D) - 1)); T;) {
          ...
          let U = v[P(h, C)].val,
            ...
            Y = Math.max(y, U, z, _);
          f.push({
            triangles: [c.get(R(p, m, C, r)), c.get(R(h, m, C, r)), c.get(R(h, p, C, r)), n],
            val: Y,
          });
        }
      }
  ...
  return { n: r, edges: v, triangles: x, tetrahedra: f };
}
var B = class {
  bits;
  words;
  ...
  pivot() {
    for (let t = this.words - 1; t >= 0; t--) {
      let e = this.bits[t];
      if (e) return (t << 5) + (31 - Math.clz32(e));
    }
    return -1;
  }
  toSparse() {
...
function q(i, t, e) {
  ...
  for (let b = 0; b < s; b++) {
    for (let l = 0; l < r; l++) X(i[b], t[l]) <= e && u[b].push(l);
    if (O(i[b]) <= e) for (let l = 0; l < s; l++) u[b].push(r + l);
  }
  for (let b = 0; b < r; b++) {
    let l = s + b;
    for (let d = 0; d < r; d++) O(t[d]) <= e && u[l].push(d);
    for (let d = 0; d < s; d++) u[l].push(r + d);
  }
  let g = new Int32Array(o).fill(-1);
  function v(b, l) {
    for (let d of u[b]) if (!l[d] && ((l[d] = 1), g[d] < 0 || v(g[d], l))) return ((g[d] = b), !0);
    return !1;
  }
  for (let b = 0; b < a; b++) {
...
function Z(i, t, e, s) {
  if (i.length === 0 && t.length === 0) return 0;
  let r = 0,
    a = e;
  if (!q(i, t, a)) return 1 / 0;
  if (q(i, t, r)) return 0;
  for (; a - r > s;) {
    let o = (r + a) / 2;
    q(i, t, o) ? (a = o) : (r = o);
...
function tt(i, t) {
  ...
  let e = [...i].sort((a, o) => a - o),
    s = [...t].sort((a, o) => a - o),
    r = 0;
  for (let a = 0; a < e.length; a++) {
    let o = Math.abs(e[a] - s[a]);
    o > r && (r = o);
  }
  return r;
}
function K(i, t, e = 0, s = 1e6, r = 1e-6) {
  let a = i.filter((l) => l.dim === e && l.death >= 0).map((l) => [l.birth, l.death]),
    o = t.filter((l) => l.dim === e && l.death >= 0).map((l) => [l.birth, l.death]),
    u = i.filter((l) => l.dim === e && l.death === -1).map((l) => l.birth),
    g = t.filter((l) => l.dim === e && l.death === -1).map((l) => l.birth);
  if (u.length !== g.length) return 1 / 0;
  let v = tt(u, g),
    b = Z(a, o, s, r);
  return Math.max(v, b);
}
...
function et(i, t, e = 1 / 0, s = 2) {
  ...
  for (let c = 0; c < o.length; c++) {
    ...
    for (P.loadFromNumbers(f.edges); ;) {
      ...
      if (h < 0) {
        ((b[n] = c),
          (l[c] = P.toSparse()),
          f.val > a[n].val && d.push({ birth: a[n].val, death: f.val, dim: 1 }));
        break;
      }
      let p = l[h];
      if (p === null) break;
  ...
  if (s >= 3 && x > 0) {
    ...
    for (let m = 0; m < o.length; m++) l[m] !== null && l[m].length === 0 && (c[m] = 1);
    let f = new Int32Array(o.length).fill(-1),
    ...
    for (let m = 0; m < u.length; m++) {
      ...
      for (h.loadFromNumbers(y.triangles); ;) {
        ...
        if (k < 0) {
          ((f[w] = m),
            (n[m] = h.toSparse()),
            y.val > o[w].val && A.push({ birth: o[w].val, death: y.val, dim: 2 }));
          break;
        }
        let S = n[k];
        if (S === null) break;
    ...
    let p = new Uint8Array(o.length);
    for (let m = 0; m < o.length; m++) f[m] >= 0 && (p[m] = 1);
...
function rt(i, t, e, s = 1) {
  ...
  for (let c = 0; c < r; c++) ((v[c] = c), (b[c] = i[c]));
  v.sort((c, f) => b[c] - b[f]);
  let l = [];
  for (let c = 0; c < t; c++)
    for (let f = 0; f < e - 1; f++) {
      let n = c * e + f,
        h = c * e + f + 1;
      l.push({ u: n, v: h, val: Math.max(i[n], i[h]), origIdx: l.length });
    }
  for (let c = 0; c < t - 1; c++)
    for (let f = 0; f < e; f++) {
      let n = c * e + f,
  ...
  let d = new Int32Array(u);
  for (let c = 0; c < l.length; c++) d[l[c].origIdx] = c;
  let P = [];
  for (let c = 0; c < t - 1; c++)
    for (let f = 0; f < e - 1; f++) {
      let n = c * e + f,
  ...
  let { h0Pairs: A, cycleEdges: E } = j(r, l),
    x = [];
  if (s >= 1 && g > 0) {
    let c = new Int32Array(u).fill(-1),
    ...
    for (let h = 0; h < g; h++) {
      ...
      for (n.loadFromNumbers(p.edges); ;) {
        ...
        if (y < 0) {
          ((c[m] = h),
            (f[h] = n.toSparse()),
            p.val > l[m].val && x.push({ birth: l[m].val, death: p.val, dim: 1 }));
          break;
        }
        let w = f[y];
        if (w === null) break;
    ...
    for (let h = 0; h < u; h++) E[h] && c[h] < 0 && x.push({ birth: l[h].val, death: -1, dim: 1 });
  }
  return { pairs: [...A, ...x], dims: { height: t, width: e } };
}
function V(i) {
  let t = [],
    e = [],
      ...
      : u.dim === 1
        ...
        : u.dim === 2
          ? u.death < 0
            ...
            : r.push(u)
          : o.push(u);
  return { h0: t, h1finite: e, h1essential: s, h2finite: r, h2essential: a, higher: o };
}
function nt(i) {
  let t = [];
  (t.push("# persistence pairs: dim birth death"), t.push(`# total pairs: ${i.length}`));
  for (let e of i) {
    let s = e.death < 0 ? "inf" : e.death.toFixed(6);
...
function it(i) {
  let t = ["dim,birth,death"];
  for (let e of i) t.push(`${e.dim},${e.birth},${e.death < 0 ? -1 : e.death}`);
  return t.join(`
`);
}
function ot(i) {
...
function at(i) {
  let t = V(i),
    e = 0,
    s = 1 / 0;
  for (let r of i) (r.birth < s && (s = r.birth), r.death > e && r.death >= 0 && (e = r.death));
  return {
    total: i.length,
    h0: t.h0.length,
